...
impl ProgOp {
    ...
    fn colour(&self, colour: bool) -> String {
...
/// ProgFmtElem and ProfFmt enums - used to convert RPN program to infix style
...
impl ProgFmtElem {
    ...
    fn colour(&self, colour: bool) -> String {
        match self {
            ProgFmtElem::Op(o) => o.colour(colour),
            ProgFmtElem::Term(t) => t.colour(colour)
        }
    }
...
enum ProgFmt {
    Number(u32),
    AddSub(Vec<ProgFmtElem>),
    MulDiv(Vec<ProgFmtElem>),
}
...
impl ProgFmt {
    ...
    fn colour(&self, colour: bool) -> String {
        match self {
            ProgFmt::AddSub(elems) | ProgFmt::MulDiv(elems) => {
                elems.iter().map(|e| {
                    match e {
                        ProgFmtElem::Term(_) => {
                            format!("({})", e.colour(colour))
                        }
                        _ => {
                            e.colour(colour)
                        }
            ...
            ProgFmt::Number(n) => {
                ProgOp::Number(*n as u8).colour(colour)
            }
        }
    }
...
#[derive(Debug)]
enum ProgEntity {
    Card(u8),        // A card element
    CommAdd(u8),     // A string of additions. Highest card index stored
    CommMul(u8),     // A string of multiplications. Highest card index stored
    Multi            // A complex term
}
...
impl Program {
                        ...
                        ProgEntity::Card(c1) => {
                            match n2 {
                                ProgEntity::Card(c2) => {
                                    ...
                                    stack.push(ProgEntity::CommAdd(c2));
                                }
                        ...
                        ProgEntity::Card(c1) => {
                            match n2 {
                                ProgEntity::Card(c2) => {
                                    ...
                                    // Only include if first number is lower than the second
                                    if c2 < c1 { return false }
                                    stack.push(ProgEntity::CommMul(c2));
                                }
    ...
    fn equation(&self, numbers: &[u32], colour: bool) -> String {
        ...
        for op in self.instructions.iter() {
            match op {
                ProgOp::Number(x) => stack.push(ProgFmt::Number(numbers[*x as usize])),
                ProgOp::OpAdd | ProgOp::OpSub => {
                    let n1 = stack.pop().unwrap();
                    let n2 = stack.pop().unwrap();
                    ...
                    match &n2 {
                        ProgFmt::Number(num2) => {
                            match &n1 {
                                ProgFmt::Number(num1) => {
                                    // Number +/- Number
                                    ops = vec![
                                        ProgFmtElem::Op(ProgOp::Number(*num2 as u8)),
                                        ProgFmtElem::Op(*op),
                                        ProgFmtElem::Op(ProgOp::Number(*num1 as u8))
                                    ];
                                }
                                ProgFmt::AddSub(ops1) => {
                                    // Number +/- AddSub
                                    ops = [ProgFmtElem::Op(ProgOp::Number(*num2 as u8))].iter().cloned()
                                        .chain([ProgFmtElem::Op(*op)])
                                        .chain(ops1.iter().cloned())
                                        .collect();
                                }
                                ProgFmt::MulDiv(_) => {
                                    // Number +/- MulDiv
                        ...
                        ProgFmt::AddSub(ops2) => {
                            match &n1 {
                                ProgFmt::Number(num1) => {
                                    // AddSub +/- Number
                                    ops = ops2.iter().cloned()
                                        .chain([ProgFmtElem::Op(*op)])
                                        .chain([ProgFmtElem::Op(ProgOp::Number(*num1 as u8))])
                                        .collect();
                                }
                                ProgFmt::AddSub(ops1) => {
                                    // AddSub +/- AddSub
                        ...
                        ProgFmt::MulDiv(_) => {
                            match &n1 {
                                ProgFmt::Number(num1) => {
                                    // MulDiv +/- Number
                                    ops = [ProgFmtElem::Term(n2)].iter().cloned()
                                        .chain([ProgFmtElem::Op(*op)])
                                        .chain([ProgFmtElem::Op(ProgOp::Number(*num1 as u8))])
                                        .collect();
                                }
                    ...
                    stack.push(ProgFmt::AddSub(ops));
                },
                ProgOp::OpMul | ProgOp::OpDiv => {
                    let n1 = stack.pop().unwrap();
                    let n2 = stack.pop().unwrap();
                    ...
                    stack.push(ProgFmt::MulDiv(ops));
                }
            }
        }

        stack.pop().unwrap().colour(colour)
    }
